...
export function ItemDetails() {
  ...
  useEffect(() => {
    if (postId && !loading && myMemberId) {
      checkIfLiked()
    }
  }, [postId, loading, myMemberId])

  const checkIfLiked = async () => {
    const token = sessionStorage.getItem('token')
    if (!token || !postId || !myMemberId) return

    try {
      const response = await axios.get(`http://localhost:8080/unknownPaw/api/posts/likes`, {
        params: {
          memberId: myMemberId,
          page: 0,
          size: 100
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const response = await axios[method](`http://localhost:8080/unknownPaw/api/posts/${postId}/like`, undefined, {
