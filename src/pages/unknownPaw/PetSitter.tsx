export function PetSitter() {
  return (
    <>
      <head>
        <title>Pet Sitter - Classified Ads and Listing Website Template.</title>
        <meta name="description" content="" />
      </head>

      <body>
        {/* <!-- Start Items Grid Area --> */}
        <section className="items-grid section custom-padding">
          <div className="container">
            <div className="row">
              <div className="col-12">
                <div className="section-title">
                  <h2 className="wow fadeInUp" data-wow-delay=".4s">
                    Pet Sitter
                  </h2>
                  <p className="wow fadeInUp" data-wow-delay=".6s">
                    서비스를 제안하고 제공해보세요!
                  </p>
                </div>
              </div>
            </div>
            <div className="single-head">
              <div className="row">
                <div className="col-lg-4 col-md-6 col-12">
                  {/* <!-- Start Single Grid --> */}
                  <div className="single-grid wow fadeInUp" data-wow-delay=".2s">
                    <div className="image">
                      <a href="item-details.html" className="thumbnail">
                        <img src="assets/images/items-grid/img1.jpg" alt="#" />
                      </a>
                      <div className="author">
                        <div className="author-image">
                          <a href="javascript:void(0)">
                            <img src="assets/images/items-grid/author-1.jpg" alt="#" />
                            <span>Smith jeko</span>
                          </a>
                        </div>
                        <p className="sale">For Sale</p>
                      </div>
                    </div>
                    <div className="content">
                      <div className="top-content">
                        <a href="javascript:void(0)" className="tag">
                          Mobile Phones
                        </a>
                        <h3 className="title">
                          <a href="item-details.html">Apple Iphone X</a>
                        </h3>
                        <p className="update-time">Last Updated: 1 hours ago</p>
                        <ul className="rating">
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <a href="javascript:void(0)">(35)</a>
                          </li>
                        </ul>
                        <ul className="info-list">
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-map-marker"></i> New York, US
                            </a>
                          </li>
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-timer"></i> Feb 18, 2023
                            </a>
                          </li>
                        </ul>
                      </div>
                      <div className="bottom-content">
                        <p className="price">
                          Start From: <span>$200.00</span>
                        </p>
                        <a href="javascript:void(0)" className="like">
                          <i className="lni lni-heart"></i>
                        </a>
                      </div>
                    </div>
                  </div>
                  {/* <!-- End Single Grid --> */}
                </div>
                <div className="col-lg-4 col-md-6 col-12">
                  {/* <!-- Start Single Grid --> */}
                  <div className="single-grid wow fadeInUp" data-wow-delay=".4s">
                    <div className="image">
                      <a href="item-details.html" className="thumbnail">
                        <img src="assets/images/items-grid/img2.jpg" alt="#" />
                      </a>
                      <div className="author">
                        <div className="author-image">
                          <a href="javascript:void(0)">
                            <img src="assets/images/items-grid/author-2.jpg" alt="#" />
                            <span>Alex Jui</span>
                          </a>
                        </div>
                        <p className="sale">For Sale</p>
                      </div>
                    </div>
                    <div className="content">
                      <div className="top-content">
                        <a href="javascript:void(0)" className="tag">
                          Real Estate
                        </a>
                        <h3 className="title">
                          <a href="item-details.html">Amazing Room for Rent</a>
                        </h3>
                        <p className="update-time">Last Updated: 2 hours ago</p>
                        <ul className="rating">
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <a href="javascript:void(0)">(20)</a>
                          </li>
                        </ul>
                        <ul className="info-list">
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-map-marker"></i> Dallas, Washington
                            </a>
                          </li>
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-timer"></i> Jan 7, 2023
                            </a>
                          </li>
                        </ul>
                      </div>
                      <div className="bottom-content">
                        <p className="price">
                          Start From: <span>$450.00</span>
                        </p>
                        <a href="javascript:void(0)" className="like">
                          <i className="lni lni-heart"></i>
                        </a>
                      </div>
                    </div>
                  </div>
                  {/* <!-- End Single Grid --> */}
                </div>
                <div className="col-lg-4 col-md-6 col-12">
                  {/* <!-- Start Single Grid --> */}
                  <div className="single-grid wow fadeInUp" data-wow-delay=".6s">
                    <div className="image">
                      <a href="item-details.html" className="thumbnail">
                        <img src="assets/images/items-grid/img3.jpg" alt="#" />
                      </a>
                      <div className="author">
                        <div className="author-image">
                          <a href="javascript:void(0)">
                            <img src="assets/images/items-grid/author-3.jpg" alt="#" />
                            <span>Devid Milan</span>
                          </a>
                        </div>
                        <p className="sale">For Sale</p>
                      </div>
                      <p className="item-position">
                        <i className="lni lni-bolt"></i> Featured
                      </p>
                    </div>
                    <div className="content">
                      <div className="top-content">
                        <a href="javascript:void(0)" className="tag">
                          Mobile Phones
                        </a>
                        <h3 className="title">
                          <a href="item-details.html">Canon SX Powershot D-SLR</a>
                        </h3>
                        <p className="update-time">Last Updated: 3 hours ago</p>
                        <ul className="rating">
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <a href="javascript:void(0)">(55)</a>
                          </li>
                        </ul>
                        <ul className="info-list">
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-map-marker"></i> New York, US
                            </a>
                          </li>
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-timer"></i> Mar 18, 2023
                            </a>
                          </li>
                        </ul>
                      </div>
                      <div className="bottom-content">
                        <p className="price">
                          Start From: <span>$700.00</span>
                        </p>
                        <a href="javascript:void(0)" className="like">
                          <i className="lni lni-heart"></i>
                        </a>
                      </div>
                    </div>
                  </div>
                  {/* <!-- End Single Grid --> */}
                </div>
                <div className="col-lg-4 col-md-6 col-12">
                  {/* <!-- Start Single Grid --> */}
                  <div className="single-grid wow fadeInUp" data-wow-delay=".2s">
                    <div className="image">
                      <a href="item-details.html" className="thumbnail">
                        <img src="assets/images/items-grid/img4.jpg" alt="#" />
                      </a>
                      <div className="author">
                        <div className="author-image">
                          <a href="javascript:void(0)">
                            <img src="assets/images/items-grid/author-4.jpg" alt="#" />
                            <span>Jesia Jully</span>
                          </a>
                        </div>
                        <p className="sale">For Sale</p>
                      </div>
                    </div>
                    <div className="content">
                      <div className="top-content">
                        <a href="javascript:void(0)" className="tag">
                          Vehicles
                        </a>
                        <h3 className="title">
                          <a href="item-details.html">BMW 5 Series GT Car</a>
                        </h3>
                        <p className="update-time">Last Updated: 4 hours ago</p>
                        <ul className="rating">
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <a href="javascript:void(0)">(35)</a>
                          </li>
                        </ul>
                        <ul className="info-list">
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-map-marker"></i> New York, US
                            </a>
                          </li>
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-timer"></i> Apr 12, 2023
                            </a>
                          </li>
                        </ul>
                      </div>
                      <div className="bottom-content">
                        <p className="price">
                          Start From: <span>$1000.00</span>
                        </p>
                        <a href="javascript:void(0)" className="like">
                          <i className="lni lni-heart"></i>
                        </a>
                      </div>
                    </div>
                  </div>
                  {/* <!-- End Single Grid --> */}
                </div>
                <div className="col-lg-4 col-md-6 col-12">
                  {/* <!-- Start Single Grid --> */}
                  <div className="single-grid wow fadeInUp" data-wow-delay=".4s">
                    <div className="image">
                      <a href="item-details.html" className="thumbnail">
                        <img src="assets/images/items-grid/img5.jpg" alt="#" />
                      </a>
                      <div className="author">
                        <div className="author-image">
                          <a href="javascript:void(0)">
                            <img src="assets/images/items-grid/author-5.jpg" alt="#" />
                            <span>Thomas Deco</span>
                          </a>
                        </div>
                        <p className="sale">For Sale</p>
                      </div>
                      <p className="item-position">
                        <i className="lni lni-bolt"></i> Featured
                      </p>
                    </div>
                    <div className="content">
                      <div className="top-content">
                        <a href="javascript:void(0)" className="tag">
                          Apple
                        </a>
                        <h3 className="title">
                          <a href="item-details.html">Apple Macbook Pro 13 Inch</a>
                        </h3>
                        <p className="update-time">Last Updated: 5 hours ago</p>
                        <ul className="rating">
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <a href="javascript:void(0)">(35)</a>
                          </li>
                        </ul>
                        <ul className="info-list">
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-map-marker"></i> Louis, Missouri, US
                            </a>
                          </li>
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-timer"></i> May 25, 2023
                            </a>
                          </li>
                        </ul>
                      </div>
                      <div className="bottom-content">
                        <p className="price">
                          Start From: <span>$550.00</span>
                        </p>
                        <a href="javascript:void(0)" className="like">
                          <i className="lni lni-heart"></i>
                        </a>
                      </div>
                    </div>
                  </div>
                  {/* <!-- End Single Grid --> */}
                </div>
                <div className="col-lg-4 col-md-6 col-12">
                  {/* <!-- Start Single Grid --> */}
                  <div className="single-grid wow fadeInUp" data-wow-delay=".6s">
                    <div className="image">
                      <a href="item-details.html" className="thumbnail">
                        <img src="assets/images/items-grid/img6.jpg" alt="#" />
                      </a>
                      <div className="author">
                        <div className="author-image">
                          <a href="javascript:void(0)">
                            <img src="assets/images/items-grid/author-6.jpg" alt="#" />
                            <span>Jonson zack</span>
                          </a>
                        </div>
                        <p className="sale">For Sale</p>
                      </div>
                    </div>
                    <div className="content">
                      <div className="top-content">
                        <a href="javascript:void(0)" className="tag">
                          Restaurant
                        </a>
                        <h3 className="title">
                          <a href="item-details.html">Cream Restaurant</a>
                        </h3>
                        <p className="update-time">Last Updated: 7 hours ago</p>
                        <ul className="rating">
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <i className="lni lni-star-filled"></i>
                          </li>
                          <li>
                            <a href="javascript:void(0)">(20)</a>
                          </li>
                        </ul>
                        <ul className="info-list">
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-map-marker"></i> New York, US
                            </a>
                          </li>
                          <li>
                            <a href="javascript:void(0)">
                              <i className="lni lni-timer"></i> Feb 18, 2023
                            </a>
                          </li>
                        </ul>
                      </div>
                      <div className="bottom-content">
                        <p className="price">
                          Start From: <span>$500.00</span>
                        </p>
                        <a href="javascript:void(0)" className="like">
                          <i className="lni lni-heart"></i>
                        </a>
                      </div>
                    </div>
                  </div>
                  {/* <!-- End Single Grid --> */}
                </div>
              </div>
            </div>
          </div>
        </section>
        {/* <!-- /End Items Grid Area --> */}
      </body>
    </>
  )
}
