  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (category === 'community') {
        // 커뮤니티 게시글은 다른 엔드포인트 사용
        const dto = {
          title: postData.title,
          content: postData.content,
          communityCategory: postData.serviceCategory // 커뮤니티 카테고리 (GENERAL, EVENT 등)
        }

        console.log('최종 전송 DTO:', JSON.stringify(dto, null, 2))

        const postRes = await fetch(
          `/api/community/${postData.serviceCategory}/register?memberId=${memberId}`,
          {
            method: 'POST',
            headers: {
